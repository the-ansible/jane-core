...
import { createDefaultBreakers, type CircuitBreakers } from './circuit-breaker.js';
...
export class SafetyGate {
  ...
  setNats(nats: NatsConnection): void {
    this.nats = nats;
  }

  private publishAlert(alertType: string, details: Record<string, unknown>): void {
    if (!this.nats) return;
    try {
      ...
      this.nats.publish(
        'communication.internal.safety-alert',
        sc.encode(payload)
      );
    } catch { /* ignore */ }
  }
  ...
  canSend(now = Date.now()): SafetyCheckResult {
    ...
    if (outboundStatus.exceeded) {
      ...
      if (outboundStatus.alertOnly) {
        log('RATE ALERT: ' + msg, { limiter: 'outbound_messages' });
        this.publishAlert('rate-limit-exceeded', { limiter: 'outbound_messages' });
      } else {
        reasons.push(msg);
      }
    }
    ...
    if (this.breakers.memory.isUnderPressure()) reasons.push('Memory pressure');
  ...
  recordSend(now = Date.now()): void {
    ...
    if (flooded) log('FLOOD DETECTED');
  }

  canCallLlm(eventType: string, now = Date.now()): SafetyCheckResult {
    const reasons: string[] = [];
    if (this.paused) reasons.push('System paused by manual override');
    const claudeStatus = this.limiters.llmClaude.status(now);
    if (claudeStatus.exceeded) {
      const msg = `LLM rate limit exceeded (${claudeStatus.current}/${claudeStatus.limit} per window)`;
      if (claudeStatus.alertOnly) {
        log('RATE ALERT: ' + msg, { limiter: 'llm_claude' });
        this.publishAlert('rate-limit-exceeded', { limiter: 'llm_claude' });
      } else {
        reasons.push(msg);
      }
    }
  ...
  canProcess(now = Date.now()): SafetyCheckResult {
    ...
    if (totalStatus.exceeded) {
      ...
      if (totalStatus.alertOnly) {
        log('RATE ALERT: ' + msg, { limiter: 'total_events' });
        this.publishAlert('rate-limit-exceeded', { limiter: 'total_events' });
      } else {
        reasons.push(msg);
      }
    }
