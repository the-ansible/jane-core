...
export async function getActivePlan(): Promise<{ name: string; config: ContextPlanConfig }> {
  ...
  const { rows } = await db.query<PlanRow>(
    `SELECT name, config FROM ${SCHEMA}.comm_plans WHERE is_active = TRUE LIMIT 1`
  );
...
export async function listPlans(): Promise<PlanRow[]> {
  const { rows } = await db.query<PlanRow>(
    `SELECT name, config, is_active, description, created_at, updated_at FROM ${SCHEMA}.comm_plans ORDER BY created_at`
  );
  return rows;
}
...
export async function setActivePlan(name: string): Promise<void> {
  await db.query(`UPDATE ${SCHEMA}.comm_plans SET is_active = FALSE, updated_at = NOW()`);
  const { rows } = await db.query<{ name: string }>(
    `UPDATE ${SCHEMA}.comm_plans SET is_active = TRUE, updated_at = NOW() WHERE name = $1 RETURNING name`,
    [name]
  );
  if (rows.length === 0) {
