 ...
 *
 * Raw messages from the summary boundary to now are always included.
...
async function storeSummary(record: SummaryRecord): Promise<void> {
  await db.query(
    `INSERT INTO ${SCHEMA}.comm_summaries
...
async function logAssembly(params: {
  ...
  const id = uuidv7();
  await db.query(
    `INSERT INTO ${SCHEMA}.comm_assembly_log
  ...
  while (rawCount > plan.rawSummarizationThreshold) {
    ...
    try {
      ...
      console.log(JSON.stringify({
        ...
        component: 'comm.context.assembler',
        sessionId,
        error: String(err),
